...
def add_Petrolpump_data(Registration_No,Petrolpump_Name,Company_Name,Opening_Year,State,City):
    c.execute(f'insert into Petrolpump values {Registration_No,Petrolpump_Name,Company_Name,Opening_Year,State,City}' )
    mydb.commit()

def add_Owners_data(Owner_Name, Contact_NO, DOB, Gender, Address, Partnership):
    c.execute(f'INSERT INTO Owners VALUES {Owner_Name, Contact_NO, DOB, Gender, Address, Partnership}')
    mydb.commit()
...
def add_Employee_data(Employee_ID, Emp_Name,  Emp_Gender,   Designation,  DOB, Salary,  Emp_Address, Email_ID , Petrolpump_No, Manager_ID):
    c.execute(f'insert into Employee values {Employee_ID, Emp_Name,  Emp_Gender,   Designation,  DOB, Salary,  Emp_Address, Email_ID , Petrolpump_No, Manager_ID}')
    mydb.commit()

def add_Customer_data(Customer_Code, C_Name , Phone_No  , Email_ID , Gender,  City , Age):
    c.execute(f'INSERT INTO Customers VALUES {Customer_Code , C_Name , Phone_No  , Email_ID , Gender,  City , Age}')
    mydb.commit() 

def add_Invoice_data(Invoice_No , Date  ,Time ,  Payment_Type , Fuel_Amount , Fuel_Type , Discount  , Total_Price , Customer_Code):
    c.execute(f'INSERT INTO Invoice Values {Invoice_No , Date  ,Time ,  Payment_Type , Fuel_Amount , Fuel_Type , Discount  , Total_Price , Customer_Code}')
    mydb.commit()

def add_Tanker_data(Tanker_ID  , Capacity,  pressure,  Fuel_ID , Fuel_Amount, Fuel_Name , Fuel_Price , Petrolpump_No):
    c.execute(f'INSERT INTO Tanker Values {Tanker_ID  , Capacity,  pressure,  Fuel_ID , Fuel_Amount, Fuel_Name , Fuel_Price , Petrolpump_No}')
    mydb.commit()
...
def view_only_Tanker_ID():
    c.execute("select Tanker_ID from Tanker_ID")
    data = c.fetchall()
    return data
...
def get_all_info_Invoice(selected_Invoice):
    c.execute('select * from Invoice_No where Invoice_No="{}"'.format(selected_Invoice))
    data = c.fetchall()
    return data
...
def edit_Invoice_data( new_Date  ,new_Time , new_Payment_Type , new_Fuel_Amount , new_Fuel_Type , new_Discount  ,new_Total_Price , new_Customer_Code, Invoice_No):
    c.execute("update Invoice set Date=%s  ,Time=%s ,  Payment_Type=%s , Fuel_Amount=%s , Fuel_Type=%s , Discount=%s  , Total_Price=%s , Customer_Code=%s where Invoice_No=%s", (new_Date  ,new_Time , new_Payment_Type , new_Fuel_Amount , new_Fuel_Type , new_Discount  ,new_Total_Price , new_Customer_Code, Invoice_No))
    mydb.commit()
    data = c.fetchall()
    return data
